...
def add_command():
    ...
    listing.insert(END, 
                    (title_text.get(), author_text.get(),year_text.get(), isbn_text.get(),shelf_text.get(),raw_text.get()))
    clear()
...
def f():
    ...
    raw_text = StringVar()
    global entry6
    entry6 = Entry(upper_frame, textvariable = raw_text,bg="powder blue",fg='green',font="Times 15 bold",relief=RAISED,bd=4)
...
#is not working, an error will pop up saying "device_id undefined" 
        with sr.Microphone(sample_rate = sample_rate, chunk_size = chunk_size) as source:
            ...
            try:
                ...
                if(key==1):
                    ...
                    sql_select_Query = "select * from Book where title=%s"
                    # sql_select_Query=sql_select_Query.encode('utf-8')
                    cursor = connection.cursor()
                    cursor.execute(sql_select_Query,(text,))
                    records = cursor.fetchall()
                    # print(records)
                    listing.delete(0, END)
                    listing.insert(END, records)
                elif(key==2):
                    authortext.set(text)
                    connection = pymysql.connect(host='localhost',
                                         database='mydb',
                    ...
                    sql_select_Query = "select * from Book where author=%s"
                    # sql_select_Query=sql_select_Query.encode('utf-8')
                    cursor = connection.cursor()
                    cursor.execute(sql_select_Query,(text,))
                    records = cursor.fetchall()
                    ...
                    sql_select_Query = "select * from Book where isbn=%s"
                    # sql_select_Query=sql_select_Query.encode('utf-8')
                    cursor = connection.cursor()
                    cursor.execute(sql_select_Query,(text,))
                    records = cursor.fetchall()
    ...
    titletext = StringVar()
    global entry7
    entry7 = Entry(bottom_frame, textvariable =titletext ,state="readonly", width=12,bg="powder blue",fg='green',font="Times 15 bold",relief=RAISED,bd=4)
    entry7.grid(row = 10, column = 0,padx=5,pady=5,sticky='nswe')

    button7 = Button(bottom_frame, 
                    text = "TITLE", 
                    width = 12, 
                    command =lambda:speaking_function(1) ,bg="powder blue",fg='green',font="Times 15 bold")
    button7.grid(row = 9, column = 0,pady=5,padx=5,sticky='nswe')
    
    global authortext
    authortext = StringVar()
    global entry8
    entry8 = Entry(bottom_frame, textvariable =authortext ,state="readonly", width=12,bg="powder blue",fg='green',font="Times 15 bold",relief=RAISED,bd=4)
    entry8.grid(row = 10, column = 1,padx=5,pady=5,sticky='nswe')

    button8 = Button(bottom_frame, 
                    text = "AUTHOR", 
                    width = 12, 
                    command =lambda:speaking_function(2) ,bg="powder blue",fg='green',font="Times 15 bold")
    button8.grid(row = 9, column = 1,pady=5,padx=5,sticky='nswe')

    global isbntext
    isbntext = StringVar()
    global entry9
    entry9 = Entry(bottom_frame, textvariable =isbntext ,state="readonly", width=12,bg="powder blue",fg='green',font="Times 15 bold",relief=RAISED,bd=4)
    entry9.grid(row = 10, column = 2,padx=5,pady=5,sticky='nswe')

    button9 = Button(bottom_frame, 
                    text = "ISBN", 
                    width = 12, 
                    command =lambda:speaking_function(3) ,bg="powder blue",fg='green',font="Times 15 bold")
    button9.grid(row = 9, column = 2,pady=5,padx=5,sticky='nswe')
    
    global searchtext
    searchtext = StringVar()
    global entry10
    entry10 = Entry(bottom_frame, textvariable =searchtext ,state="readonly", width=25,bg="powder blue",fg='green',font="Times 15 bold",relief=RAISED,bd=4)
    entry10.grid(row = 10, column = 3,padx=5,pady=5,sticky='nswe')

    button10 = Button(bottom_frame, 
                    text = "SEARCH ON INTERNET", 
                    width = 12,
                    command =lambda:speaking_function(4) ,bg="powder blue",fg='green',font="Times 15 bold")
